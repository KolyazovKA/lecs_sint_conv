...
class Analyzator:
	...
	def fill_table(self, numb_str, position, char):
		...
		if char in self.numbs_of_str and ['Число', numb_str, position, char] not in self.table:
			self.table.add_row(['Число', numb_str, position, char])
		if char in "(" and ['Открывающая круглая скобка', numb_str, position, char] not in self.table:
			self.table.add_row(['Открывающая круглая скобка', numb_str, position, char])
		if char in ")" and ['Закрывающая круглая скобка', numb_str, position, char] not in self.table:
			self.table.add_row(['Закрывающая круглая скобка', numb_str, position, char])
		if char in "{" and ['Открывающая фигурная скобка', numb_str, position, char] not in self.table:
			self.table.add_row(['Открывающая фигурная скобка', numb_str, position, char])
		if char in "}" and ['Закрывающая фигурная скобка', numb_str, position, char] not in self.table:
			self.table.add_row(['Закрывающая фигурная скобка', numb_str, position, char])
		if char in ":=" and ['Оператор присваивания', numb_str, position, char] not in self.table:
			self.table.add_row(['Оператор присваивания', numb_str, position, char])
		if char in "*" and ['Оператор умножения', numb_str, position, char] not in self.table:
			self.table.add_row(['Оператор умножения', numb_str, position, char])
		if char in "/" and ['Оператор деления', numb_str, position, char] not in self.table:
			self.table.add_row(['Оператор деления', numb_str, position, char])
		if char in "+" and ['Оператор сложения', numb_str, position, char] not in self.table:
			self.table.add_row(['Оператор сложения', numb_str, position, char])
		if char in "-" and ['Оператор вычитания', numb_str, position, char] not in self.table:
			self.table.add_row(['Оператор вычитания', numb_str, position, char])
		if char in ";" and ['Точка с запятой', numb_str, position, char] not in self.table:
		...
		for i in range(len(self.input_data)):
			for j in range(len(self.input_data[i + 1])):
				...
				if not var[0].isalpha() and not var[0] == "_":
					self.broken_vars.append(var)
					self.err_message.append(f"Ошибка в наименовании переменной {var} "
					      f"на строке {i + 1} в выражении {j + 1}\n")
					continue
